        ...
        <div class="card">
            <header class=" card-header noborder">
                <div class="justify-end flex gap-3 items-center flex-wrap">
                    {{-- Create Button start--}}
                    @can('customer-request create')
                        <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2 !px-3" href="{{ route('customer-requests.create') }}">
                            <iconify-icon icon="ic:round-plus" class="text-lg mr-1"></iconify-icon>
                            {{ __('New') }}
                        </a>
                    @endcan
                    {{--Refresh Button start--}}
                    <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2.5" href="{{ route('customer-requests.index') }}">
                        <iconify-icon icon="mdi:refresh" class="text-xl "></iconify-icon>
                    </a>
                </div>
                <div class="justify-center flex flex-wrap sm:flex items-center lg:justify-end gap-3">
                    <div class="relative w-full sm:w-auto flex items-center">
                        <form id="searchForm" method="get" action="{{ route('customer-requests.index') }}">
                    ...
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                @forelse ($customerRequests as $customerRequest)
                                    <tr class="border border-slate-100 dark:border-slate-900 relative">
                                        ...
                                        <td class="table-td">
                                            <div class="action-btns space-x-2 flex">
                                                {{-- view--}}
                                                @can('customer-request show')
                                                    <a class="action-btn"
                                                       href="{{ route('customer-requests.show', $customerRequest) }}">
                                                        <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                    </a>
                                                @endcan

                                                {{-- Edit --}}
                                                @can('customer-request update')
                                                    <a class="action-btn" href="{{ route('customer-requests.edit', ['customerRequest' => $customerRequest]) }}">
                                                        <iconify-icon icon="uil:edit"></iconify-icon>
                                                    </a>
                                                @endcan
                                                {{-- delete --}}
                                                @can('customer-request delete')
                                                    <form id="deleteForm{{ $customerRequest->id }}" method="POST" action="{{ route('customer-requests.destroy', $customerRequest) }}" class="cursor-pointer">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a class="action-btn" onclick="sweetAlertDelete(event, 'deleteForm{{ $customerRequest->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'customer-requests.index'" :data="$customerRequests" />
                        </div>
                    </div>
                </div>
            </div>
